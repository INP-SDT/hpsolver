...
class OperatDtN:
    ...
    def __init__(self, size):
        ...
        self.mat = {
            k: np.zeros((size, size)) for k in self.MAT_KEYS
        }

        self.b_1 = np.zeros((size, 1))
        self.b_2 = np.zeros((size, 1))

        self.u_1 = np.zeros((size, 1))
        self.u_2 = np.zeros((size, 1))
...
class MergerDtN:
    ...
    def __init__(self, size):
        ...
        self.q_1 = np.zeros((size, size))
        self.q_2 = np.zeros((size, size))
        self.q12 = np.zeros((size, size))

        self.r12 = np.zeros((size, 1))
        self.u12 = np.zeros((size, 1))
    ...
    def build_bridge(self):
        ...
        self.q12 = np.linalg.solve(
            self.west.a22 - self.east.a11, np.eye(self.size)
        )
